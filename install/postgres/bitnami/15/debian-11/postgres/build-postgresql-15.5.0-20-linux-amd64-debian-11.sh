echo Docker image: docker.io/bitnami/debian-base-buildpack:bullseye-r5 , Deobfusction from BUILD.txt https://downloads.bitnami.com/files/stacksmith/postgresql-15.5.0-20-linux-amd64-debian-11.tar.gz
echo Commands: && source patches.sh
apt-get update -y
DEBIAN_FRONTEND="noninteractive" apt-get install -y --no-install-recommends libedit-dev libicu-dev uuid-dev flex libpcre3-dev liblz4-dev bzip2 sqlite3 libsqlite3-dev cmake libkrb5-dev libyaml-dev libzstd-dev libbz2-dev libz-dev
...
mkdir -p /srp-tools && \
    curl -Lo /srp-tools/srp https://artifactory.eng.vmware.com/artifactory/srp-tools-generic-local/srpcli/0.16.6-20231222173311-81862cf-278/linux-amd64/srp && \
...
curl https://ftp.postgresql.org/pub/source/v15.5/postgresql-15.5.tar.gz --output /tmp/sources/postgresql/postgresql-15.5.tar.gz
tar --no-same-owner -C /bitnami/blacksmith-sandox/ -xf /tmp/sources/postgresql/postgresql-15.5.tar.gz
cd /bitnami/blacksmith-sandox/postgresql-15.5
...
make  --jobs=5
make  install --jobs=5
wget https://github.com/OSGeo/gdal/releases/download/v3.8.2/gdal-3.8.2.tar.gz -O /tmp/sources/gdal/gdal-3.8.2.tar.gz
tar --no-same-owner -C /bitnami/blacksmith-sandox/ -xf /tmp/sources/gdal/gdal-3.8.2.tar.gz
cd /bitnami/blacksmith-sandox/gdal-3.8.2
cmake . -DCMAKE_INSTALL_PREFIX:PATH=/opt/bitnami/postgresql -DCMAKE_INSTALL_LIBDIR=lib -DCMAKE_BUILD_TYPE=Release -DBUILD_PYTHON_BINDINGS:BOOL=OFF -DGDAL_USE_EXTERNAL_LIBS:BOOL=OFF -DGDAL_USE_PROJ=ON -DGDAL_USE_GEOS=ON -B/bitnami/blacksmith-sandox/gdal-3.8.2/out
cd out && make  --jobs=5
...
mvn  clean package
java  -jar /bitnami/blacksmith-sandox/pljava-1_6_6/pljava-packaging/target/pljava-pg15.jar
...
make  --jobs=5
make  install --jobs=5
wget https://github.com/pgaudit/pgaudit/archive/1.7.0.tar.gz -O /tmp/sources/pgaudit/pgaudit-1.7.0.tar.gz
tar --no-same-owner -C /bitnami/blacksmith-sandox/ -xf /tmp/sources/pgaudit/pgaudit-1.7.0.tar.gz
cd /bitnami/blacksmith-sandox/pgaudit-1.7.0
make  USE_PGXS=1 --jobs=5
make  USE_PGXS=1 install --jobs=5
wget https://github.com/citusdata/pg_auto_failover/archive/v2.1.tar.gz -O /tmp/sources/pgautofailover/pg_auto_failover-2.1.tar.gz
tar --no-same-owner -C /bitnami/blacksmith-sandox/ -xf /tmp/sources/pgautofailover/pg_auto_failover-2.1.tar.gz
cd /bitnami/blacksmith-sandox/pg_auto_failover-2.1
make  --jobs=5
make  install --jobs=5
...
cd /bitnami/blacksmith-sandox/pgbackrest-release-2.49/src
./configure  --prefix=/opt/bitnami/postgresql
make  --jobs=5
make  install --jobs=5
...
cmake  . -DCMAKE_INSTALL_PREFIX=/opt/bitnami/common -B./out
make  --jobs=5
make  install --jobs=5
...
strip  "/opt/bitnami/postgresql/lib/libecpg_compat.so.3.15"
strip  "/opt/bitnami/postgresql/lib/libecpg.so.6.15"
strip  "/opt/bitnami/postgresql/lib/libgdal.so.34.3.8.2"
...
strip  "/opt/bitnami/postgresql/lib/libpgtypes.so.3.15"
strip  "/opt/bitnami/postgresql/lib/libpljava-so-1.6.6.so"
strip  "/opt/bitnami/postgresql/lib/libpq.so.5.15"
